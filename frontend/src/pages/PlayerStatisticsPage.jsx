...
import { fetchPlayerSelfPerformanceHistory } from "../api";
...
export default function PlayerStatisticsPage({ activeTeam }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError("");
    void fetchPlayerSelfPerformanceHistory(teamId)
      .then((data) => {
        if (!cancelled) {
          setPayload(data);
        }
  ...
  const chartPoints = useMemo(() => {
    ...
    const points = plotted.map((row, index) => {
      ...
      return {
        ...
        value,
        label: `${formatMatchDate(row.scheduledDate)} #${index + 1}`,
      };
    });
      ...
      <section className="teams-page-shell">
        <header className="teams-page-header">
          <div className="teams-page-heading">
            <p className="teams-page-kicker">Player</p>
            <h1>Statistics</h1>
            <p className="teams-page-subtitle">Pick one of your teams in the toolbar to view your performance history.</p>
          </div>
        </header>
      </section>
    );
      ...
      <header className="teams-page-header">
        <div className="teams-page-heading">
          ...
          <p className="teams-page-subtitle">
            Your personal match history and progress for <strong>{activeTeam?.name}</strong>.
          </p>
        </div>
      </header>

      <section className="vc-panel">
        {loading ? <p className="vc-modal__muted">Loading your statistics…</p> : null}
        {error ? <p className="vc-modal__error">{error}</p> : null}
        {!loading && !error && !rows.length ? (
          <p className="vc-modal__muted">
            No performance records yet. Once match stats are recorded, your history will appear here.
          </p>
        ) : null}
        ...
        {!loading && !error && rows.length ? (
          <div style={{ display: "grid", gap: "0.75rem" }}>
            <div>
              <strong>{payload?.player?.name || "Player"}</strong>
              <p className="vc-modal__muted" style={{ margin: "0.2rem 0 0" }}>{payload?.player?.email || "—"}</p>
            </div>
            <p className="vc-modal__muted" style={{ margin: 0 }}>
              Matches tracked: <strong>{rows.length}</strong>
              {" · "}
            ...
            {chartPoints ? (
              <section style={{ border: "1px solid #e4e7ec", borderRadius: "10px", padding: "0.7rem 0.8rem", background: "#fbfcfe" }}>
                <h3 style={{ margin: "0 0 0.35rem", fontSize: "0.94rem" }}>{selectedMetric.label} progress over time</h3>
                <svg viewBox={`0 0 ${chartPoints.width} ${chartPoints.height}`} style={{ width: "100%", height: 220 }}>
                  {chartPoints.yTicks.map((tick) => (
                    <g key={`yt-${tick.y}`}>
                      <line x1={chartPoints.chartLeft} y1={tick.y} x2={chartPoints.chartRight} y2={tick.y} stroke="#e5eaf1" strokeWidth="1" />
                  ...
                  <line x1={chartPoints.chartLeft} y1={chartPoints.chartBottom} x2={chartPoints.chartRight} y2={chartPoints.chartBottom} stroke="#cfd8e3" strokeWidth="1" />
                  <line x1={chartPoints.chartLeft} y1={chartPoints.chartTop} x2={chartPoints.chartLeft} y2={chartPoints.chartBottom} stroke="#cfd8e3" strokeWidth="1" />
                  ...
                  {chartPoints.points.map((point, index) => (
                    chartPoints.rotate ? (
                      <text key={`x-${index}`} x={point.x} y={chartPoints.chartBottom + 18} fill="#6b7280" fontSize="10" textAnchor="end" transform={`rotate(-30 ${point.x} ${chartPoints.chartBottom + 18})`}>
                        {point.label}
                      </text>
                    ) : (
                      <text key={`x-${index}`} x={point.x} y={chartPoints.chartBottom + 18} fill="#6b7280" fontSize="10" textAnchor="middle">
                        {point.label}
                      </text>
                    )
                  ))}
