...
class SharedMatchRequestFlowTests(TestCase):
    ...
    def test_elimination_bracket_winner_advances_automatically(self):
        ...
        tournament = Tournament.objects.create(
            club=fx["club"],
            created_by=fx["director"],
            name="Bracket Test",
            tournament_type=Tournament.TournamentType.BRACKET,
            number_of_teams=4,
            pool_count=0,
            teams_per_pool=0,
        ...
        tournament.teams.set([fx["team_a"], fx["team_b"], team_c])
        ...
        next_fixture = TournamentFixture.objects.create(
            tournament=tournament,
            stage_type=TournamentFixture.StageType.BRACKET,
            round_number=2,
            round_label="Final",
        ...
        next_fixture.refresh_from_db()
        self.assertEqual(next_fixture.home_team_id, fx["team_a"].id)
        self.assertEqual(next_fixture.placeholder_home_label, "")
        self.assertIsNotNone(next_fixture.training_session_id)
        self.assertEqual(next_fixture.training_session.team_id, fx["team_a"].id)
        self.assertEqual(next_fixture.training_session.opponent_team_id, team_c.id)

    def test_two_team_tie_break_uses_head_to_head_first(self):
        ordered = _sort_tournament_rows(
            [
                {
                    "team_id": 1,
                    "wins": 2,
        ...
        self.assertEqual([row["team_id"] for row in ordered], [2, 1])

    def test_hybrid_tournament_promotes_top_qualifiers_into_seeded_bracket(self):
        fx = self._fixture()
        team_c = Team.objects.create(club=fx["club"], name="Shared Team C")
        team_d = Team.objects.create(club=fx["club"], name="Shared Team D")
        team_e = Team.objects.create(club=fx["club"], name="Shared Team E")
        team_f = Team.objects.create(club=fx["club"], name="Shared Team F")

        coach_c = User.objects.create_user(email="sm-coach-c@example.com", password="StrongPassword123!")
        coach_d = User.objects.create_user(email="sm-coach-d@example.com", password="StrongPassword123!")
        coach_e = User.objects.create_user(email="sm-coach-e@example.com", password="StrongPassword123!")
        coach_f = User.objects.create_user(email="sm-coach-f@example.com", password="StrongPassword123!")
        player_c = User.objects.create_user(email="sm-player-c@example.com", password="StrongPassword123!", date_of_birth=date(2008, 3, 1))
        player_d = User.objects.create_user(email="sm-player-d@example.com", password="StrongPassword123!", date_of_birth=date(2008, 4, 1))
        player_e = User.objects.create_user(email="sm-player-e@example.com", password="StrongPassword123!", date_of_birth=date(2008, 5, 1))
        player_f = User.objects.create_user(email="sm-player-f@example.com", password="StrongPassword123!", date_of_birth=date(2008, 6, 1))

        TeamMembership.objects.add_member(user=coach_c, team=team_c, role=TeamRole.COACH)
        TeamMembership.objects.add_member(user=coach_d, team=team_d, role=TeamRole.COACH)
        TeamMembership.objects.add_member(user=coach_e, team=team_e, role=TeamRole.COACH)
        TeamMembership.objects.add_member(user=coach_f, team=team_f, role=TeamRole.COACH)
        TeamMembership.objects.add_member(user=player_c, team=team_c, role=TeamRole.PLAYER)
        TeamMembership.objects.add_member(user=player_d, team=team_d, role=TeamRole.PLAYER)
        TeamMembership.objects.add_member(user=player_e, team=team_e, role=TeamRole.PLAYER)
        TeamMembership.objects.add_member(user=player_f, team=team_f, role=TeamRole.PLAYER)

        create_response = self.client.post(
            reverse("core:club-tournaments", kwargs={"club_id": fx["club"].id}),
            data=json.dumps(
                {
                    "name": "Hybrid Seed Test",
            ...
            content_type="application/json",
            HTTP_AUTHORIZATION=f"Bearer {generate_auth_token(fx['director'])}",
        )
        self.assertEqual(create_response.status_code, 201)
        tournament = Tournament.objects.get(name="Hybrid Seed Test")

        pool_fixtures = {
            tuple(sorted((fixture.home_team_id, fixture.away_team_id))): fixture.training_session
            for fixture in TournamentFixture.objects.filter(
                tournament=tournament,
                stage_type=TournamentFixture.StageType.POOL,
                training_session_id__isnull=False,
            ).select_related("training_session")
        }

        results = [
            (fx["team_a"], fx["team_b"], 25, 20),
            (team_c, fx["team_a"], 25, 15),
            (team_c, fx["team_b"], 25, 15),
            (team_d, team_e, 25, 20),
            (team_f, team_d, 25, 20),
            (team_f, team_e, 25, 20),
        ]
        team_players = {
            fx["team_a"].id: (fx["player_a"], fx["coach_a"]),
            fx["team_b"].id: (fx["player_b"], fx["coach_b"]),
            team_c.id: (player_c, coach_c),
            team_d.id: (player_d, coach_d),
            team_e.id: (player_e, coach_e),
            team_f.id: (player_f, coach_f),
        }

        for first_team, second_team, first_points, second_points in results:
            session = pool_fixtures[tuple(sorted((first_team.id, second_team.id)))]
            session_team_player, session_team_coach = team_players[session.team_id]
            opponent_player, opponent_coach = team_players[session.opponent_team_id]
            session_team_points = first_points if session.team_id == first_team.id else second_points
            opponent_points = second_points if session.team_id == first_team.id else first_points
            MatchPlayerStat.objects.create(
                training_session=session,
                player=session_team_player,
                points_scored=session_team_points,
                updated_by=session_team_coach,
            )
            MatchPlayerStat.objects.create(
                training_session=session,
                player=opponent_player,
                points_scored=opponent_points,
                updated_by=opponent_coach,
            )
            response = self.client.post(
                reverse("core:end-match", kwargs={"match_id": session.id}),
                data=json.dumps({}),
                content_type="application/json",
                HTTP_AUTHORIZATION=f"Bearer {generate_auth_token(session_team_coach)}",
            )
            self.assertEqual(response.status_code, 200)

        detail_response = self.client.get(
            reverse(
                "core:club-tournament-detail",
                kwargs={"club_id": fx["club"].id, "tournament_id": tournament.id},
            ),
            HTTP_AUTHORIZATION=f"Bearer {generate_auth_token(fx['director'])}",
        )
        self.assertEqual(detail_response.status_code, 200)
        fixtures = detail_response.json()["tournament"]["fixtures"]
        semi_finals = [fixture for fixture in fixtures if fixture["stage_type"] == "bracket" and fixture["round_label"] == "Semi-finals"]
        self.assertEqual(len(semi_finals), 2)
        self.assertEqual(
            [(fixture["home_team"]["name"], fixture["away_team"]["name"]) for fixture in semi_finals],
