...
def _tournament_fixture_session_status(session):
    if session is None:
        return None
...
def _tournament_fixture_display_status(fixture):
    session_status = _tournament_fixture_session_status(fixture.training_session)
    if session_status:
        return session_status
    if fixture.placeholder_home_label == "BYE" or fixture.placeholder_away_label == "BYE":
        return "bye"
    return "placeholder"


def _tournament_fixture_winner_name(session):
    if session is None or not session.match_ended_at:
        return None

    winner_team = _match_winner_team(session)
    if winner_team is not None:
        return winner_team.name

    if session.opponent_team_id:
        team_points = _match_points_by_team(session)
        if team_points.get(session.team_id) == team_points.get(session.opponent_team_id):
            return "Draw"

    return None


def _serialize_tournament_fixture(fixture):
    session = fixture.training_session
    return {
        "id": fixture.id,
        "stage_type": fixture.stage_type,
        "stage_type_label": fixture.get_stage_type_display(),
        ...
        "home_team": _serialize_tournament_team(fixture.home_team) if fixture.home_team_id else None,
        "away_team": _serialize_tournament_team(fixture.away_team) if fixture.away_team_id else None,
        "placeholder_home_label": fixture.placeholder_home_label,
        "placeholder_away_label": fixture.placeholder_away_label,
        "scheduled_date": (
            fixture.scheduled_date.isoformat()
            if fixture.scheduled_date
            else (session.scheduled_date.isoformat() if session else None)
        ),
        "start_time": (
            fixture.start_time.strftime("%H:%M")
            if fixture.start_time
            else (session.start_time.strftime("%H:%M") if session else None)
        ),
        "training_session_id": session.id if session else None,
        "training_session_status": _tournament_fixture_session_status(session),
        "display_status": _tournament_fixture_display_status(fixture),
        "winner_name": _tournament_fixture_winner_name(session),
        "match_request_status": session.match_request_status if session else None,
    }


def _tournament_runtime_status(tournament, fixtures=None):
    if tournament.status == Tournament.Status.CANCELLED:
        return ("cancelled", "Cancelled")

    if fixtures is None:
        fixtures = list(
            tournament.fixtures.select_related("training_session").all()
        )

    if not fixtures:
        return ("ongoing", "Ongoing")

    has_playable_fixture = False
    for fixture in fixtures:
        session = fixture.training_session
        if session is not None:
            has_playable_fixture = True
...
def _serialize_tournament(tournament):
    ...
    return {
        ...
        "tournament_type_label": tournament.get_tournament_type_display(),
        "number_of_teams": tournament.number_of_teams,
        "pool_count": tournament.pool_count,
        "teams_per_pool": tournament.teams_per_pool,
        "teams_qualifying_per_pool": tournament.teams_qualifying_per_pool,
        ...
        "model_status": tournament.status,
        "model_status_label": tournament.get_status_display(),
        "available_rounds": [
            {"round_number": round_number, "round_label": round_label}
            for round_number, round_label in sorted({(row.round_number, row.round_label) for row in fixtures})
        ],
...
def _add_minutes_to_time(start_value: time, minutes: int) -> time:
    return (datetime.combine(date.today(), start_value) + timedelta(minutes=minutes)).time()


def _round_robin_rounds(participants):
...
def _build_seeded_bracket_pairs(participants):
    if len(participants) < 2:
        return []
    bracket_size = 1 << (len(participants) - 1).bit_length()
    slots = list(participants) + [None] * (bracket_size - len(participants))
    pair_count = bracket_size // 2
    return [(slots[index], slots[-(index + 1)]) for index in range(pair_count)]


def _bracket_round_label(total_rounds: int, round_index: int) -> str:
    remaining_rounds = total_rounds - round_index
    ...
    return f"Bracket Round {round_index}"


def _create_tournament_match_session(
    *,
    tournament,
    home_team,
    away_team,
    ...
    end_time = _add_minutes_to_time(start_time, tournament.match_duration_minutes)
    notes_parts = [f"Tournament: {tournament.name}", f"Round: {round_label}"]
    if pool_name:
        notes_parts.append(f"Pool: {pool_name}")
    notes_parts.append(f"Scoring: {tournament.scoring_format}")
...
def _parse_tournament_payload(payload, club):
    ...
    tournament_type = (payload.get("tournament_type") or "").strip()
    if tournament_type not in Tournament.TournamentType.values:
        raise ValidationError({"tournament_type": "Choose a valid tournament type."})

    raw_team_ids = payload.get("team_ids")
    if not isinstance(raw_team_ids, list):
        raise ValidationError({"team_ids": "Select at least 2 teams."})

    team_ids = []
    for raw_value in raw_team_ids:
        try:
            team_id = int(raw_value)
        except (TypeError, ValueError) as exc:
            raise ValidationError({"team_ids": "Team ids must be numeric."}) from exc
    ...
    teams_by_id = {
        team.id: team
        for team in Team.objects.filter(club=club, id__in=team_ids, status=Team.Status.ACTIVE).order_by("id")
    }
    teams = [teams_by_id[team_id] for team_id in team_ids if team_id in teams_by_id]
    if len(teams) != len(team_ids):
        raise ValidationError({"team_ids": "One or more selected teams are invalid for this club."})

    raw_number_of_teams = payload.get("number_of_teams", len(team_ids))
    try:
        number_of_teams = int(raw_number_of_teams)
    except (TypeError, ValueError) as exc:
        raise ValidationError({"number_of_teams": "Use a whole number."}) from exc
    if number_of_teams != len(team_ids):
        raise ValidationError({"number_of_teams": "Selected teams must match the tournament team count."})

    try:
        match_duration_minutes = int(payload.get("match_duration_minutes", 90))
    except (TypeError, ValueError) as exc:
        raise ValidationError({"match_duration_minutes": "Use a whole number of minutes."}) from exc
    if match_duration_minutes < 15 or match_duration_minutes > 240:
        raise ValidationError({"match_duration_minutes": "Use a duration between 15 and 240 minutes."})

    scoring_format = (payload.get("scoring_format") or "").strip()
    if not scoring_format:
        raise ValidationError({"scoring_format": "Scoring format is required."})
    ...
    if start_date < timezone.localdate():
        raise ValidationError({"start_date": "Tournament start date cannot be in the past."})

    start_time = _parse_time_hhmm(payload.get("start_time") or "18:00", "start_time")

    pool_count = 0
    teams_per_pool = 0
    teams_qualifying_per_pool = 0
    if tournament_type in {Tournament.TournamentType.POOLS, Tournament.TournamentType.HYBRID}:
        try:
            pool_count = int(payload.get("pool_count"))
            teams_per_pool = int(payload.get("teams_per_pool"))
        except (TypeError, ValueError) as exc:
            raise ValidationError({"pool_count": "Pool count and teams per pool are required."}) from exc
        if pool_count < 1:
            raise ValidationError({"pool_count": "Use at least 1 pool."})
        if teams_per_pool < 2:
            raise ValidationError({"teams_per_pool": "Each pool must have at least 2 teams."})
        if pool_count * teams_per_pool != len(teams):
            raise ValidationError(
                {"teams_per_pool": "Pool structure must exactly match the selected number of teams."}
            )
        if tournament_type == Tournament.TournamentType.HYBRID:
            try:
                teams_qualifying_per_pool = int(payload.get("teams_qualifying_per_pool"))
            except (TypeError, ValueError) as exc:
                raise ValidationError(
                    {"teams_qualifying_per_pool": "Choose how many teams qualify from each pool."}
                ) from exc
            if teams_qualifying_per_pool < 1:
                raise ValidationError(
                    {"teams_qualifying_per_pool": "At least 1 team per pool must qualify."}
                )
            if teams_qualifying_per_pool > teams_per_pool:
                raise ValidationError(
                    {
                        "teams_qualifying_per_pool": (
                            "Qualified teams per pool cannot be greater than teams per pool."
                        )
                    }
                )
    ...
    return {
        ...
        "teams_qualifying_per_pool": teams_qualifying_per_pool,
...
def _generate_pool_stage(tournament, teams):
    if tournament.pool_count < 1 or tournament.teams_per_pool < 2:
        return 0

    max_pool_rounds = 0
    for pool_index in range(tournament.pool_count):
        pool_teams = teams[
            pool_index * tournament.teams_per_pool:(pool_index + 1) * tournament.teams_per_pool
        ]
        pool = TournamentPool.objects.create(
            tournament=tournament,
            name=f"Pool {chr(ord('A') + pool_index)}",
            pool_order=pool_index + 1,
        )
        rounds = _round_robin_rounds(pool_teams)
        max_pool_rounds = max(max_pool_rounds, len(rounds))
        for round_index, pairings in enumerate(rounds, start=1):
            round_date = tournament.start_date + timedelta(days=round_index - 1)
            for fixture_order, (home_team, away_team) in enumerate(pairings, start=1):
                if home_team is None or away_team is None:
                    actual_team = home_team or away_team
                    TournamentFixture.objects.create(
                        ...
                        home_team=actual_team if home_team is not None else None,
                        away_team=actual_team if away_team is not None and home_team is None else None,
                        placeholder_home_label="BYE" if home_team is None else "",
                        placeholder_away_label="BYE" if away_team is None else "",
                        scheduled_date=round_date,
                        start_time=tournament.start_time,
                    )
                    continue
                session = _create_tournament_match_session(
                    tournament=tournament,
                    home_team=home_team,
                    away_team=away_team,
                    scheduled_date=round_date,
    ...
    return max_pool_rounds


def _tournament_draw_sort_value(*parts):
    digest = hashlib.sha256(":".join(str(part) for part in parts).encode("utf-8")).hexdigest()
    return int(digest[:16], 16)


def _pool_team_draw_key(tournament, pool, team):
    return _tournament_draw_sort_value("pool", tournament.id, pool.id, team.id)


def _overall_team_draw_key(tournament, team):
    return _tournament_draw_sort_value("overall", tournament.id, team.id)


def _tournament_pool_team_rows(pool):
    fixtures = list(
        pool.fixtures.select_related("training_session", "home_team", "away_team")
        .prefetch_related("training_session__match_player_stats")
        .order_by("round_number", "fixture_order", "id")
    )
    teams = {}
    for fixture in fixtures:
    ...
    rows = {
        team_id: {
            ...
            "head_to_head": {},
            "draw_key": _pool_team_draw_key(pool.tournament, pool, team),
        }
    ...
    for fixture in fixtures:
        ...
        if (
            session is None
            or fixture.home_team_id is None
            or fixture.away_team_id is None
            or not session.match_ended_at
        ):
            continue

        team_points = _match_points_by_team(session)
        home_points = team_points.get(fixture.home_team_id)
        away_points = team_points.get(fixture.away_team_id)
        if home_points is None or away_points is None:
            continue

        home_row = rows[fixture.home_team_id]
        away_row = rows[fixture.away_team_id]
        home_row["matches_played"] += 1
        away_row["matches_played"] += 1
        home_row["points_for"] += int(home_points)
        home_row["points_against"] += int(away_points)
        away_row["points_for"] += int(away_points)
        away_row["points_against"] += int(home_points)
        home_row["point_differential"] = home_row["points_for"] - home_row["points_against"]
        away_row["point_differential"] = away_row["points_for"] - away_row["points_against"]

        if home_points > away_points:
            home_row["wins"] += 1
            away_row["losses"] += 1
            home_row["head_to_head"][fixture.away_team_id] = 1
            away_row["head_to_head"][fixture.home_team_id] = -1
            ...
            home_row["draws"] += 1
            away_row["draws"] += 1
            home_row["head_to_head"][fixture.away_team_id] = 0
            away_row["head_to_head"][fixture.home_team_id] = 0

    return list(rows.values())


def _compare_tournament_rows(left, right):
    if left["wins"] != right["wins"]:
        return -1 if left["wins"] > right["wins"] else 1
    if left["point_differential"] != right["point_differential"]:
        return -1 if left["point_differential"] > right["point_differential"] else 1
    if left["points_for"] != right["points_for"]:
        return -1 if left["points_for"] > right["points_for"] else 1
    if left["draw_key"] != right["draw_key"]:
        return -1 if left["draw_key"] < right["draw_key"] else 1
    return -1 if left["team_id"] < right["team_id"] else (1 if left["team_id"] > right["team_id"] else 0)


def _sort_tournament_rows(rows):
    sorted_rows = sorted(rows, key=cmp_to_key(_compare_tournament_rows))
    grouped_rows = []
    index = 0
    while index < len(sorted_rows):
        ...
        tie_group = [current]
        index += 1
        while index < len(sorted_rows) and sorted_rows[index]["wins"] == current["wins"]:
            tie_group.append(sorted_rows[index])
            index += 1

        if len(tie_group) == 2:
            left, right = tie_group
            head_to_head_result = left["head_to_head"].get(right["team_id"])
            if head_to_head_result == -1:
                tie_group = [right, left]
            elif head_to_head_result == 1:
                tie_group = [left, right]
            else:
                tie_group = sorted(tie_group, key=cmp_to_key(_compare_tournament_rows))
        else:
            tie_group = sorted(tie_group, key=cmp_to_key(_compare_tournament_rows))

        grouped_rows.extend(tie_group)
    return grouped_rows


def _pool_fixture_is_playable(fixture):
    return fixture.home_team_id is not None and fixture.away_team_id is not None


def _pool_stage_complete(tournament):
    fixtures = list(
        TournamentFixture.objects.filter(
            tournament=tournament,
            stage_type=TournamentFixture.StageType.POOL,
        ).select_related("training_session")
    )
    playable_fixtures = [fixture for fixture in fixtures if _pool_fixture_is_playable(fixture)]
    return bool(playable_fixtures) and all(
        fixture.training_session_id and fixture.training_session.match_ended_at
        for fixture in playable_fixtures
    )


def _qualified_hybrid_rows(tournament):
    if tournament.tournament_type != Tournament.TournamentType.HYBRID or not _pool_stage_complete(tournament):
        return []

    qualified_rows = []
    for pool in tournament.pools.order_by("pool_order", "id"):
        pool_rows = _sort_tournament_rows(_tournament_pool_team_rows(pool))
        for standing_index, row in enumerate(pool_rows[: tournament.teams_qualifying_per_pool], start=1):
            qualified_rows.append(
                {
                    **row,
                    "pool_id": pool.id,
                    "pool_name": pool.name,
                    "pool_rank": standing_index,
                    "draw_key": _overall_team_draw_key(tournament, row["team"]),
                }
            )

    return _sort_tournament_rows(qualified_rows)


def _serialize_tournament_pool_standings(tournament):
    if tournament.tournament_type not in {
        Tournament.TournamentType.POOLS,
                ...
                "teams": [
                    {
                        ...
                        "qualified": (
                            tournament.tournament_type == Tournament.TournamentType.HYBRID
                            and index < tournament.teams_qualifying_per_pool
                        ),
                    }
...
def _matchup_key(home_team, away_team):
    if home_team is None or away_team is None:
        return None
    low_id, high_id = sorted((home_team.id, away_team.id))
    return (low_id, high_id)


def _register_tournament_matchup(seen_matchups, home_team, away_team):
    if home_team is None or away_team is None:
        return
    if home_team.id == away_team.id:
        raise ValidationError({"team_ids": "A team cannot play against itself."})
    matchup_key = _matchup_key(home_team, away_team)
    if matchup_key in seen_matchups:
        raise ValidationError(
            {"team_ids": "Tournament schedule cannot contain duplicate matchups."}
        )
    seen_matchups.add(matchup_key)


def _bracket_next_fixture_slot(source_fixture):
    return ("home_team", "placeholder_home_label") if source_fixture.fixture_order % 2 == 1 else (
        "away_team",
        "placeholder_away_label",
    )


def _ensure_bracket_fixture_session(fixture):
    if (
        fixture.stage_type != TournamentFixture.StageType.BRACKET
        or fixture.training_session_id
        or fixture.home_team_id is None
        or fixture.away_team_id is None
    ):
        return
    if fixture.home_team_id == fixture.away_team_id:
        return
    duplicate_fixture_exists = TournamentFixture.objects.filter(
        tournament=fixture.tournament,
        home_team_id__isnull=False,
        away_team_id__isnull=False,
    ).exclude(pk=fixture.pk).filter(
        Q(home_team=fixture.home_team, away_team=fixture.away_team)
        | Q(home_team=fixture.away_team, away_team=fixture.home_team)
    ).exists()
    if duplicate_fixture_exists:
        return
    ...
    session = _create_tournament_match_session(
        ...
        scheduled_date=fixture.scheduled_date or fixture.tournament.start_date,
        start_time=fixture.start_time or fixture.tournament.start_time,
        round_label=fixture.round_label,
    )
    fixture.training_session = session
    fixture.save(update_fields=["training_session"])


def _advance_bracket_team(source_fixture, winner_team):
    next_fixture = (
        TournamentFixture.objects.select_related("tournament", "home_team", "away_team")
        .filter(
            tournament=source_fixture.tournament,
            stage_type=TournamentFixture.StageType.BRACKET,
            round_number=source_fixture.round_number + 1,
            fixture_order=(source_fixture.fixture_order + 1) // 2,
        )
        .first()
    )
    ...
    team_field, placeholder_field = _bracket_next_fixture_slot(source_fixture)
    updated_fields = []
    if getattr(next_fixture, f"{team_field}_id") != winner_team.id:
        setattr(next_fixture, team_field, winner_team)
        updated_fields.append(team_field)
    if getattr(next_fixture, placeholder_field):
        setattr(next_fixture, placeholder_field, "")
        updated_fields.append(placeholder_field)
    if updated_fields:
        next_fixture.save(update_fields=updated_fields)

    _ensure_bracket_fixture_session(next_fixture)


def _auto_advance_bracket_byes(tournament, start_round_number):
    first_round_fixtures = TournamentFixture.objects.filter(
        tournament=tournament,
        stage_type=TournamentFixture.StageType.BRACKET,
        round_number=start_round_number,
    ).select_related("home_team", "away_team")

    for fixture in first_round_fixtures:
        teams = [fixture.home_team, fixture.away_team]
        present_teams = [team for team in teams if team is not None]
        placeholder_labels = {fixture.placeholder_home_label, fixture.placeholder_away_label}
        if len(present_teams) == 1 and "BYE" in placeholder_labels:
            _advance_bracket_team(fixture, present_teams[0])


def _match_winner_team(session):
    team_points = _match_points_by_team(session)
    context_score = team_points.get(session.team_id)
    if context_score is None:
        return None

    if session.opponent_team_id:
        opponent_score = team_points.get(session.opponent_team_id)
        if opponent_score is None or context_score == opponent_score:
            return None
        return session.team if context_score > opponent_score else session.opponent_team

    if session.opponent_final_score is None or context_score == session.opponent_final_score:
        return None
    return session.team if context_score > session.opponent_final_score else None


def _advance_tournament_bracket_winner_from_session(session):
    fixture = (
        TournamentFixture.objects.select_related("tournament", "home_team", "away_team")
        .filter(training_session=session, stage_type=TournamentFixture.StageType.BRACKET)
        .first()
    )
    if fixture is None:
        return
    ...
    _advance_bracket_team(fixture, winner_team)


def _reconcile_tournament_bracket(tournament):
    if tournament.tournament_type == Tournament.TournamentType.HYBRID:
        _seed_hybrid_bracket_from_pools(tournament)

    bracket_fixtures = list(
        TournamentFixture.objects.filter(
            tournament=tournament,
            stage_type=TournamentFixture.StageType.BRACKET,
        )
        .select_related("tournament", "home_team", "away_team", "training_session")
        .order_by("round_number", "fixture_order", "id")
    )
    if not bracket_fixtures:
        return

    first_round_number = bracket_fixtures[0].round_number
    _auto_advance_bracket_byes(tournament, first_round_number)

    for fixture in bracket_fixtures:
        session = fixture.training_session
        if session is not None and session.match_ended_at:
            _advance_tournament_bracket_winner_from_session(session)
        else:
            _ensure_bracket_fixture_session(fixture)


def _generate_bracket_stage(
    tournament,
    participants,
    *,
    ...
    seeded_pairs = _build_seeded_bracket_pairs(participants)
    bracket_size = len(seeded_pairs) * 2
    total_rounds = (bracket_size.bit_length() - 1)

    for round_index in range(1, total_rounds + 1):
        round_label = _bracket_round_label(total_rounds, round_index)
        scheduled_date = tournament.start_date + timedelta(days=start_round_number + round_index - 2)
        match_count = bracket_size // (2 ** round_index)
        for fixture_index in range(match_count):
            fixture_order = fixture_index + 1
            if round_index == 1:
                left, right = seeded_pairs[fixture_index]
                home_team = left if isinstance(left, Team) else None
                away_team = right if isinstance(right, Team) else None
                placeholder_home = "" if home_team else (left or "BYE")
                placeholder_away = "" if away_team else (right or "BYE")
                session = None
                if not placeholders_only and home_team is not None and away_team is not None:
                    if seen_matchups is not None:
                        _register_tournament_matchup(seen_matchups, home_team, away_team)
                    session = _create_tournament_match_session(
                        tournament=tournament,
                        home_team=home_team,
                        away_team=away_team,
                ...
                away_team = None
                session = None
                previous_label = _bracket_round_label(total_rounds, round_index - 1)
                placeholder_home = f"Winner of {previous_label} Match {fixture_index * 2 + 1}"
                placeholder_away = f"Winner of {previous_label} Match {fixture_index * 2 + 2}"

            TournamentFixture.objects.create(
                tournament=tournament,
                training_session=session,
                stage_type=TournamentFixture.StageType.BRACKET,
                round_number=start_round_number + round_index - 1,
                round_label=round_label,
                fixture_order=fixture_order,
                home_team=home_team,
                away_team=away_team,
                placeholder_home_label=placeholder_home,
                placeholder_away_label=placeholder_away,
                scheduled_date=scheduled_date,
                start_time=tournament.start_time,
            )

    if not placeholders_only:
        _auto_advance_bracket_byes(tournament, start_round_number)


def _hybrid_bracket_participants(tournament):
    qualifier_count = tournament.pool_count * tournament.teams_qualifying_per_pool
    return [f"Seed {seed_index}" for seed_index in range(1, qualifier_count + 1)]


def _seed_hybrid_bracket_from_pools(tournament):
    if tournament.tournament_type != Tournament.TournamentType.HYBRID:
        return

    qualified_rows = _qualified_hybrid_rows(tournament)
    if len(qualified_rows) < 2:
        return

    seeded_pairs = _build_seeded_bracket_pairs([row["team"] for row in qualified_rows])
    first_round_fixtures = list(
        TournamentFixture.objects.filter(
            tournament=tournament,
            stage_type=TournamentFixture.StageType.BRACKET,
        )
        .order_by("round_number", "fixture_order", "id")
        .select_related("home_team", "away_team", "training_session")
    )
    if not first_round_fixtures:
        return

    first_round_number = first_round_fixtures[0].round_number
    seeded_fixtures = [fixture for fixture in first_round_fixtures if fixture.round_number == first_round_number]
    if len(seeded_fixtures) != len(seeded_pairs):
        return

    for fixture, (home_team, away_team) in zip(seeded_fixtures, seeded_pairs):
        updated_fields = []
        desired_home_id = home_team.id if isinstance(home_team, Team) else None
        desired_away_id = away_team.id if isinstance(away_team, Team) else None
        desired_home_label = "" if isinstance(home_team, Team) else (home_team or "BYE")
        desired_away_label = "" if isinstance(away_team, Team) else (away_team or "BYE")

        if fixture.home_team_id != desired_home_id:
            fixture.home_team = home_team if isinstance(home_team, Team) else None
            updated_fields.append("home_team")
        if fixture.away_team_id != desired_away_id:
            fixture.away_team = away_team if isinstance(away_team, Team) else None
            updated_fields.append("away_team")
        if fixture.placeholder_home_label != desired_home_label:
            fixture.placeholder_home_label = desired_home_label
            updated_fields.append("placeholder_home_label")
        if fixture.placeholder_away_label != desired_away_label:
            fixture.placeholder_away_label = desired_away_label
            updated_fields.append("placeholder_away_label")

        if updated_fields:
            fixture.save(update_fields=updated_fields)
        _ensure_bracket_fixture_session(fixture)
...
def club_tournaments(request, club_id):
    ...
    try:
        tournament_data = _parse_tournament_payload(payload, club)
    except ValidationError as exc:
        if hasattr(exc, "message_dict"):
            return JsonResponse({"errors": exc.message_dict}, status=400)
        return JsonResponse({"errors": {"body": exc.messages}}, status=400)
    ...
    existing_tournament = (
        Tournament.objects.filter(club=club, status=Tournament.Status.GENERATED)
        .prefetch_related("fixtures__training_session")
        .order_by("-created_at", "-id")
    )
    existing_tournament = next(
        (tournament for tournament in existing_tournament if _tournament_runtime_status(tournament)[0] == "ongoing"),
        None,
    )
        ...
        return JsonResponse(
            {
                "errors": {
                    "tournament": (
                        "This club already has a generated tournament. Reuse the current one for now."
                    )
                }
            },
            status=400,
    ...
    try:
        with transaction.atomic():
            tournament = Tournament.objects.create(
                ...
                number_of_teams=tournament_data["number_of_teams"],
                pool_count=tournament_data["pool_count"],
                teams_per_pool=tournament_data["teams_per_pool"],
                teams_qualifying_per_pool=tournament_data["teams_qualifying_per_pool"],
                match_duration_minutes=tournament_data["match_duration_minutes"],
                scoring_format=tournament_data["scoring_format"],
                start_date=tournament_data["start_date"],
                start_time=tournament_data["start_time"],
                venue=tournament_data["venue"],
            )
            tournament.teams.set(tournament_data["teams"])

            last_pool_round = 0
            seen_matchups = set()
            if tournament.tournament_type in {Tournament.TournamentType.POOLS, Tournament.TournamentType.HYBRID}:
                for pool_index in range(tournament.pool_count):
                    pool_teams = tournament_data["teams"][
                        pool_index * tournament.teams_per_pool:(pool_index + 1) * tournament.teams_per_pool
                    ]
                    for pairings in _round_robin_rounds(pool_teams):
                        for home_team, away_team in pairings:
                            _register_tournament_matchup(seen_matchups, home_team, away_team)
                last_pool_round = _generate_pool_stage(tournament, tournament_data["teams"])

            if tournament.tournament_type == Tournament.TournamentType.BRACKET:
                _generate_bracket_stage(
                    tournament,
                    tournament_data["teams"],
                    start_round_number=1,
                    seen_matchups=seen_matchups,
                )
            elif tournament.tournament_type == Tournament.TournamentType.HYBRID:
                _generate_bracket_stage(
                    tournament,
                    _hybrid_bracket_participants(tournament),
                    start_round_number=last_pool_round + 1,
        ...
        if hasattr(exc, "message_dict"):
            return JsonResponse({"errors": exc.message_dict}, status=400)
        return JsonResponse({"errors": {"tournament": exc.messages}}, status=400)
...
def club_tournament_detail(request, club_id, tournament_id):
    ...
    if tournament.status == Tournament.Status.CANCELLED:
        return JsonResponse(
            {"errors": {"tournament": "This tournament is already cancelled."}},
            status=400,
        )

    session_ids = list(
        tournament.fixtures.exclude(training_session_id__isnull=True).values_list(
            "training_session_id",
            flat=True,
        )
    )
    ...
    with transaction.atomic():
        TrainingSession.objects.filter(id__in=session_ids).delete()
